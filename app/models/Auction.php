...
class Auction {
    ...
    public function deleteAuction($auction_id, $user_id): array {
        $check_sql = "SELECT id, status, winner_id, payment_status FROM auctions WHERE id = ? AND seller_id = ?";
        $stmt = $this->conn->prepare($check_sql);
        ...
        // Cannot delete paid auctions
        if ($auction['status'] == 'paid') {
            return ['success' => false, 'message' => 'Paid auctions cannot be deleted'];
        }
        
        // Cannot delete active auctions with bids
        if ($auction['status'] == 'active') {
            return ['success' => false, 'message' => 'Active auctions cannot be deleted. You can only delete ended auctions.'];
        }
        
        // Allow deletion for ended auctions or payment_pending
        if ($auction['status'] == 'ended' || 
            ($auction['status'] == 'payment_pending' && $auction['payment_status'] == 'pending')) {
        ...
        return ['success' => false, 'message' => 'This auction cannot be deleted'];
    }
...
public function checkExpiredPayments() {
    ...
    foreach ($expired_auctions as $auction) {
        ...
        // Notify the winner that payment expired and auction is closed
        $this->addNotification(
            $auction['winner_id'],
            'payment_expired',
            '⏰ Payment deadline expired',
            "You did not complete the payment for '{$auction['title']}' within 24 hours."
        );
    }
}
